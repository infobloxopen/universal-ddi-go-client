...
Other parameters are passed through a pointer to a `UICSRAPIApproveRequest` struct via the builder pattern
...
Other parameters are passed through a pointer to a `UICSRAPIDenyRequest` struct via the builder pattern
...
func main() {
	...
	apiClient := infraprovision.NewAPIClient()
	resp, r, err := apiClient.UICSRAPI.List(context.Background()).Execute()
	if err != nil {
...
Other parameters are passed through a pointer to a `UICSRAPIListRequest` struct via the builder pattern
...
Other parameters are passed through a pointer to a `UICSRAPIRevokeRequest` struct via the builder pattern
...
Other parameters are passed through a pointer to a `UICSRAPIRevoke2Request` struct via the builder pattern
