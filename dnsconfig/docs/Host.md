...
**Dfp** | Pointer to **bool** | Below _dfp_ field is deprecated and not supported anymore. The indication whether or not UniversalDDI DNS and UniversalDDI TD DFP are both active on the host will be migrated into the new _dfp_service_ field. | [optional] [readonly] 
**DfpService** | Pointer to **string** | DFP service indicates whether or not UniversalDDI DNS and UniversalDDI TD DFP are both active on the host. If so, UniversalDDI DNS will augment recursive queries and forward them to UniversalDDI TD DFP. Allowed values:  * _unavailable_: UniversalDDI TD DFP application is not available,  * _enabled_: UniversalDDI TD DFP application is available and enabled,  * _disabled_: UniversalDDI TD DFP application is available but disabled. | [optional] [readonly] 
