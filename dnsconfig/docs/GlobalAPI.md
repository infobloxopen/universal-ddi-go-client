...
func main() {
	...
	apiClient := dnsconfig.NewAPIClient()
	resp, r, err := apiClient.GlobalAPI.Read2(context.Background(), id).Execute()
	if err != nil {
...
Other parameters are passed through a pointer to a `GlobalAPIRead2Request` struct via the builder pattern
...
Other parameters are passed through a pointer to a `GlobalAPIUpdate2Request` struct via the builder pattern
